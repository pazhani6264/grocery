...
class DeliveryBoy extends Model
{
    ...
    public static function floatingCash($request)
    {
        $consumer_data = array();
        $consumer_data['consumer_key'] = request()->header('consumer-key');
        $consumer_data['consumer_secret'] = request()->header('consumer-secret');
        ...
        if ($authenticate == 1) {
            ...
            $existUser = DB::table('users')
                    ->where('id', $deliveryboy_id)->where('status', '1')
                    ->get();
            ...
            if (count($existUser)>0) {
                ...
                $amount = DB::table('floating_cash')->where([
                    ['deliveryboy_id', $deliveryboy_id],
                    ['status', 0]
                ])->sum('amount');   
                ...
                $responseData = array('success' => '1', 'data' => $floating_cash, 'floating_cash' => $amount, 'message' => 'Data has been returned successfully!');
            }
            else
            {
                $responseData = array('success' => '0', 'data' => array(), 'floating_cash' => 0, 'message' => "INvalid Delivery Boy ID.");
            }
        } else {
            $responseData = array('success' => '0', 'data' => array(), 'message' => "Unauthenticated call.");
        }
    ...
    public static function balanceAmount($request)
    {
        $consumer_data = array();
        $consumer_data['consumer_key'] = request()->header('consumer-key');
        $consumer_data['consumer_secret'] = request()->header('consumer-secret');
        ...
        if ($authenticate == 1) {
            ...
            if (count($existUser)>0) {
                ...
                $balance = DB::table('payment_withdraw')->where([
                    ['user_id', $deliveryboy_id],
                    ['status', 0]
                ])->sum('amount');

                $settlementcash = DB::table('payment_withdraw')->where([
                    ['user_id', $deliveryboy_id],
                    ['status', 0]
                ])->get(); 
                ...
                $responseData = array('success' => '1', 'data' => $settlementcash, 'settlement_cash' => $balance, 'message' => 'Data has been returned successfully!');
            }
            else
            {
                $responseData = array('success' => '0', 'data' => array(), 'settlement_cash' => 0,  'message' => "INvalid Delivery Boy ID.");
            }
        } else {
            $responseData = array('success' => '0', 'data' => array(), 'message' => "Unauthenticated call.");
        }
    ...
    public static function settlementAmount($request)
    {
        $consumer_data = array();
        $consumer_data['consumer_key'] = request()->header('consumer-key');
        $consumer_data['consumer_secret'] = request()->header('consumer-secret');
        ...
        if ($authenticate == 1) {
            ...
            if (count($existUser)>0) {
                ...
                $balance = DB::table('payment_withdraw')->where([
                    ['user_id', $deliveryboy_id],
                    ['status', 1]
                ])->whereBetween('updated_at', [$dateFrom, $dateTo])->sum('amount');

                $settlementcash = DB::table('payment_withdraw')->where([
                    ['user_id', $deliveryboy_id],
                    ['status', 1]
                ])->whereBetween('updated_at', [$dateFrom, $dateTo])->get(); 
