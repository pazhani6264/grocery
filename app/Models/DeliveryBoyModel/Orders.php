...
class Orders extends Model
{
        ...
        if ($authenticate == 1) {
            ...
            if ($delivery_boy) {
                $order = DB::table('orders')
                    ...
                    ->orderBy('orders_to_delivery_boy.orders_id', 'desc')->get();
                if (count($order) > 0) {
                    //foreach
                    $index = '0';
                    foreach ($order as $data) {
                        ...
                        //->where('orders_status_description.orders_status_id','!=', 11)
                            ...
                            ->select('orders_status_description.orders_status_name', 'orders_status.orders_status_id', 'orders_status_history.comments','orders_status.cancel_flag')
                        ...
                        $order[$index]->cancel_flag = $orders_status_history[0]->cancel_flag;
                        ...
                        foreach ($orders_status_history as $orders_status_history_data) {
                            if ($total_comments == $i && $i != 1) {
                                ...
                                $order[$index]->cancel_flag = $orders_status_history_data->cancel_flag;
                            } else {
                        ...
                        $orders_products = DB::table('orders_products')
                            ...
                            ->orderby('orders_products.orders_id', 'ASC')
    ...
    public static function changeOrderStatus($request)
    {
        ...
        if ($authenticate == 1) {
            ...
            if ($exist) {
                ...
                //orders update
                $orders_update = DB::table('orders')->where('orders_id',$orders_id)->update(['order_status_id' => $orders_status_id]);
    ...
    public static function ordersByID($request)
    {
        $consumer_data = array();
        $consumer_data = array();
        $consumer_data['consumer_key'] = request()->header('consumer-key');
