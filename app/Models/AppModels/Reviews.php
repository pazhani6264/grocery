...
class Reviews extends Model
{
 ...
 $review_control =$settings['Review'];
  ...
  public static function updatereview($request){
   ...
   public static function getreviews($request){
    ...
    public static function giveorderreview($request){
      ...
      if($authenticate==1){
          ...
          if(count($reviews)==0){
            ...
            $reviews_id = DB::table('reviews')->insertGetId([
              ...
              $responseData = array('success'=>'1', 'data'=>array(), 'message'=>'Order is reviewed successfully!');
    
            }else{
              $responseData = array('success'=>'1', 'data'=>array(), 'message'=>'You have already given the review for this order.');
            }
      ...
      public static function getorderreviews($request){
        ...
        if($authenticate==1){
            ...
            $reviews = DB::table('reviews')
                      ->join('reviews_description','reviews_description.review_id','=','reviews.reviews_id')
                      ->join('users','users.id','=','reviews.customers_id')
                      ->select('reviews.reviews_id', 'reviews.products_id', 'reviews.orders_id', 'reviews.reviews_rating as rating', 'reviews.created_at', 'reviews_description.reviews_text as comments', 'users.first_name',
                      'users.last_name', 'users.email')
                      ->where('reviews.orders_id', $orders_id)
                      ->where('reviews.customers_id', $customers_id)
                      ->where('reviews.reviews_status', 1)
                      ->where('reviews_description.language_id', $languages_id)->get();
              
            if(count($reviews) > 0){
                $responseData = array('success'=>'1', 'data'=>$reviews, 'message'=>'Order Product rating has been returned!');
            }else{
                $responseData = array('success'=>'2', 'data'=>array(), 'message'=>'Order Product is not rated yet.');
            }
