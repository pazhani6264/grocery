...
class Banner extends Model
{
 ...
 public static function getbanners($request){
  ...
  if($authenticate==1){
    ...
    $banners_type = $request->banners_type;
    if($banners_type == '')
    {
      $responseData = array('success'=>'0', 'data'=>array(),  'message'=>"Required Banner Type");
    }
    else{
        if($banners_type == 1)
        {
          $banners = DB::table('banners')
            ...
            ->select('banners_id as id', 'banners_title as title', 'banners_url as url', 'image_categories.image_id', 'image_categories.path as image', 'image_categories.path_type as path_type', 'type', 'banners_title as title')
            ->where('status', '=', '1')
          ...
          if($banners_type == 2)
          {
            $banners = DB::table('bannertwos')
            ->LeftJoin('image_categories', function ($join) {
                $join->on('image_categories.image_id', '=', 'bannertwos.banners_image')
                    ->where(function ($query) {
                        $query->where('image_categories.image_type', '=', 'THUMBNAIL')
                            ->where('image_categories.image_type', '!=', 'THUMBNAIL')
                            ->orWhere('image_categories.image_type', '=', 'ACTUAL');
