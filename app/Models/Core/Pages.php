...
public static function updatecontent($request)
{
  ...
  DB::table('pages_description')->where('page_id','=',$page_id)->update([
...
public static function addnewwebpagebuilder($request)
{
...
public static function updatepageStatus($request){
  if(!empty($request->id)){
    if($request->active=='no'){
      $status = '0';
    }elseif($request->active=='yes'){
  ...
  $updatestatus =  DB::table('pages')->where('page_id', '=', $request->id)->update([
       'page_build_status'		 =>	  $status
   ]);
  return $updatestatus;
  }
}

public static function editnewwebpagebuilder($request)
{
  $page_id      =   $request->id;
...
public static function destroyrecord($request)
{
    DB::table('pages')->where('page_id', $request->id)->delete();
    DB::table('pages_description')->where('page_id', $request->id)->delete();
