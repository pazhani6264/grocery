...
class Languages extends Model
{
    ...
    public $sortable = ['languages_id', 'name', 'code','directory','sort_order','direction','is_default','created_at','updated_at'];
        ...
        public function englishgetter(){
            $languages = Languages::leftJoin('images','images.id', '=', 'languages.image')
                ...
                ->where('languages.languages_id','=', 1)
                      ->groupby('languages_id')->get();
    ...
    public function insert($request){
        ...
        $banners = DB::table('constant_banners')->where('languages_id',1)->get();
		 $banners_two = DB::table('constant_banners_two')->where('languages_id',1)->get();
		 $banners_three = DB::table('constant_banners_three')->where('languages_id',1)->get();
		 $banners_four = DB::table('constant_banners_four')->where('languages_id',1)->get();
      ...
      $language =  DB::table('languages')->insertGetId([
        ...
        foreach ($banners as $banner) {
          DB::table('constant_banners')->insert([
		   ...
		   foreach ($banners_two as $banner2) {
          DB::table('constant_banners')->insert([
              'banners_title'  		 =>   $banner2->banners_title,
              'banners_image'			 =>	  $banner2->banners_image,
              'banners_url'	 		 =>   $banner2->banners_url,
              'status'	 			 =>   $banner2->status,
              'type'					 =>	  $banner2->type,
              'languages_id' => $language
          ]);
        }
		   ...
		   foreach ($banners_three as $banner3) {
          DB::table('constant_banners')->insert([
              'banners_title'  		 =>   $banner3->banners_title,
              'banners_image'			 =>	  $banner3->banners_image,
              'banners_url'	 		 =>   $banner3->banners_url,
              'status'	 			 =>   $banner3->status,
              'type'					 =>	  $banner3->type,
              'languages_id' => $language
          ]);
        }
		   ...
		   foreach ($banners_four as $banner4) {
          DB::table('constant_banners')->insert([
              'banners_title'  		 =>   $banner4->banners_title,
              'banners_image'			 =>	  $banner4->banners_image,
              'banners_url'	 		 =>   $banner4->banners_url,
              'status'	 			 =>   $banner4->status,
              'type'					 =>	  $banner4->type,
              'languages_id' => $language
          ]);
        }
