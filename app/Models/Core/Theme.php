...
class Theme extends Model
{
    ...
    public static function updateTopOffer($request)
    {
        ...
        if($request->style == '3' || $request->style == '4'){
            if($request->bg_image!==null){
                $type_value = $request->bg_image;
            }else{
                $type_value = $request->oldImage;
            }
        }else{
            $type_value = $request->bg_color;
        }

        DB::table('settings')->where('name', '=', 'topoffer')->update([
            'value' => $request->topstatus,
            'updated_at' => date('Y-m-d h:i:s'),
        ]);

        DB::table('settings')->where('name', '=', 'head_offer_title')->update([
            'value' => $request->head_offer_title,
            'updated_at' => date('Y-m-d h:i:s'),
        ]);

        DB::table('settings')->where('name', '=', 'head_offer_url')->update([
            'value' => $request->head_offer_url,
            'updated_at' => date('Y-m-d h:i:s'),
        ]);

        DB::table('settings')->where('name', '=', 'top_bar_title')->update([
            'value' => $request->top_bar_title,
            'updated_at' => date('Y-m-d h:i:s'),
        ]);

        DB::table('settings')->where('name', '=', 'top_bar_url')->update([
            'value' => $request->top_bar_url,
            'updated_at' => date('Y-m-d h:i:s'),
        ]);
        ...
        foreach ($languages as $languages_data) {
            ...
            if ($checkExist) {
                DB::table('top_offers')->where('language_id', '=', $languages_data->languages_id)->update([
                    ...
                    'topoffer_status' => $request->topstatus,
                    'updated_at' => date('Y-m-d h:i:s'),
                ]);
            } else {
