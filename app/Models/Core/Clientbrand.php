...
class Clientbrand extends Model
{
    ...
    public $sortable = ['id', 'is_feature', 'created_at'];
    public $sortableAs = ['title'];
    protected $table = "clientbrand";
    use Sortable;
    ...
    public function paginator()
    {
        ...
        $clientbrand = $this->sortable(['id' => 'ASC'])
            ->LeftJoin('image_categories as iconTable', function ($join) {
                $join->on('iconTable.image_id', '=', 'clientbrand.image_id')
            ...
            ->select('iconTable.path as path', 'clientbrand.*')
            ->orderBy('clientbrand.id', 'ASC')
    ...
    public function filter($request)
    {
        ...
        if ($name == "Title") {
            ...
            ->select('iconTable.path as path', 'clientbrand.*')
            ->where('clientbrand.title', 'LIKE', '%' . $param . '%')
            ->orderBy('clientbrand.id', 'ASC')
            ->paginate($commonsetting['pagination']);

        } else {
            $clientbrand =  $this->paginator();
    ...
    {
        DB::table('clientbrand')->where('id', $request->id)->delete();
