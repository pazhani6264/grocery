...
class Shipping extends Model
{
    ...
    public function addMyAddress($request)
    {
        ...
        $defaultset = $request->default_set;
        ...
        if (!empty($customers_id)) {
            ...
            if($default_set == 1){
                DB::table('user_to_address')->where('user_id',auth()->guard('customer')->user()->id)->where('address_book_id', '!=', $address_book_id)->where('is_default', '=', 1)->update(['is_default' => 0]);
                DB::table('user_to_address')->insert(['user_id' => auth()->guard('customer')->user()->id, 'address_book_id' => $address_book_id, 'is_default' => $default_set]);
            }else{
                DB::table('user_to_address')->insert(['user_id' => auth()->guard('customer')->user()->id, 'address_book_id' => $address_book_id, 'is_default' => $default_set]);
            }
        }
    ...
    public function getShippingAddress($address_id)
    {
        ...
        ->select(
            ...
            'address_book.entry_phone as entry_phone',
            'address_book.entry_cc_code as entry_cc_code',
            'address_book.landmark as landmark',
    ...
    public function getShippingAddress_Default()
    {
        $addresses = DB::table('user_to_address')
        ->leftjoin('address_book', 'address_book.address_book_id', '=', 'user_to_address.address_book_id')
        ->leftJoin('countries', 'countries.countries_id', '=', 'address_book.entry_country_id')
