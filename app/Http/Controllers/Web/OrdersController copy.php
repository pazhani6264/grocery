             ...
             if ($shipping_methods->methods_type_link == 'upsShipping' and $shipping_methods->status == '1') {
                ...
                if(!empty(session('shipping_address'))) 
                {
                    ...
                    if (count($pricebykm) > 0) {
                        ...
                        $kmarray = array();
                        foreach ($pricebykm as $km) {
                            $kmarray[] = $km->km_id;
                        }

                        //$kmar = "'" . implode ( "', '", $kmarray ) . "'";

                        $kmar = $kmarray;
                        //$myArray = explode(',', $kmar);
                        //print_r($kmar);die();

                        $priceByWeight = DB::table('products_shipping_rates_km')->where('weight_from', '<=', $weight)->where('weight_to', '>=', $weight)->where('km_status', 1)
                        //->whereRaw('FIND_IN_SET(' . $kmarray . ',km_id)')
                        //->where('km_id', $km->km_id)
                        ->whereIn('km_id', $kmar)
                        ->get();

                        //print_r($priceByWeight);


                        if (!empty($priceByWeight) and count($priceByWeight) > 0) {
                            $price = $priceByWeight[0]->km_price;
                        } else {
                            $price = 0;
                        }
