...
{
    public function __construct(Reports $reports, Setting $setting, Categories $category, Customers $customers, Products $products,Currency $currency, Coupon $coupon, DeliveryBoys $deliveryBoys) {
    ...
    public function walletCustomers(Request $request)
    {
        if(isset($request->customers_id) || isset($request->orderid) ){
            $result['price'] = $this->reports->customersWalletTotal($request);
            $result['reports'] = $this->reports->customersWallet($request);
        }
        ...
        //print_r($result['reports']); die();

        $title = array('pageTitle' => 'Customers wallet report');
        $result['customers'] = $this->Customers->getter();
        $result['currency'] = $this->Currency->getter();
        ...
        $result['setting'] = $myVar->getSetting();
        $result['commonContent'] = $myVar->Setting->commonContent();
        return view("admin.reports.walletCustomers", $title)->with('result', $result);
    }


    public function salesOrdersPrint(Request $request)
    {

        $title = array('pageTitle' => Lang::get("labels.CustomerOrdersTotal"));
        
        $result['reports'] = $this->reports->salesprintReport($request);
        $result['price'] = $this->reports->customersReportTotal($request);
    ...
    public function couponReport(Request $request)
    {
        ...
        return view("admin.reports.couponreportnew", $title)->with('result', $result)->with('coupons', $coupons);
    ...
    public function couponreportfilter(Request $request)
    {
        ...
        switch ($name) {
            ...
            case 'Description':
                ...
                break;
            default:
            $coupons = Coupon::sortable()->where('code', 'LIKE', '%' . $param . '%')
                    ->orderBy('created_at', 'DESC')
                    ->paginate(7);
        ...
        return view("admin.reports.couponreportnew", $title)->with('result', $result)->with('coupons', $coupons)->with('name', $name)->with('param', $param);
    }
...
/*     public function salesreport(Request $request)
    ...
    public function salesReportDetails($id)
    {
         $ordersData = $this->reports->salesdetail($id);
         return view("admin.reports.salesreport_details")->with('data', $ordersData);
    }

    public function productSalesReport(Request $request)
    {
       $title = array('pageTitle' => 'Product Sales Report');
       if (isset($request->dateRange)) {
            $range = explode('-', $request->dateRange);
            ...
            $result['dateFrom'] = date('d-m-Y ', strtotime($startdate));
            $result['dateTo'] = date('d-m-Y ', strtotime($enddate));
            $result['dateRange'] = $request->dateRange;
        }
        else{
            $result['dateFrom'] = date('d-m-Y ');
            $result['dateTo'] = date('d-m-Y ');
        ...
        $result['reports'] = $this->reports->productsalesreport($request);
        $result['subCategories'] = $subCategories;
        $myVar = new SiteSettingController();
        $result['setting'] = $myVar->getSetting();
        $result['commonContent'] = $myVar->Setting->commonContent();
        return view("admin.reports.productsalesreport", $title)->with('result', $result);
    }

    public function salesProductPrint(Request $request)
    {
        $title = array('pageTitle' => 'Product Sales Report');
        $result['reports'] = $this->reports->productsalesreport($request);
        $myVar = new SiteSettingController();
        $result['setting'] = $myVar->getSetting();
        $result['commonContent'] = $myVar->Setting->commonContent();
        return view("admin.reports.productsalesreport_print", $title)->with('result', $result);
    }
 ...
 */
         ...
         /* sakthi 8-7-22 */
        $result['products'] = $this->Products->getreportproduct();   
        ...
        if ($reportBase == 'this_month') {
            ...
            for ($j = 1; $j <= 12; $j++) {
                ...
                $customers = DB::table('users')
                    ->whereBetween('created_at', [$dateFrom, $dateTo])
                    ->count(); 
                $saleData[$j - 1]['date'] = date('M', strtotime($dateFrom));
                $saleData[$j - 1]['totalSale'] = $totalSale;
                $saleData[$j - 1]['productQuantity'] = $producQuantity;
                $saleData[$j - 1]['customers'] = $customers;
            }
           ...
           $diff = abs(strtotime($dateFrom) - strtotime($dateTo));
            ...
            if ($diff == 0 && $days == 0 && $years == 0 && $months == 0) {
               ...
               for ($j = 1; $j <= $totalDays; $j++) {
                   ...
                   $dateTo     = date($selectedYear . '-' . $selectedMonth . '-' . $dateLimitFrom . ' 23:59:59', time());
                   //print $dateFrom .'<br>';
    ...
    public function loyaltyreport(Request $request)
    {
       $title = array('pageTitle' => Lang::get("labels.CustomerOrdersTotal"));
        ...
        $earn_points = $this->reports->view_loyaltyreport();
        //print_r($result['reports']);die();
        ...
        return view("admin.reports.loyaltyreport", $title)->with('result', $result)->with('earn_points', $earn_points); 
    }
    public function loyaltyreportfilter(Request $request)
    {
        $title = array('pageTitle' => Lang::get("labels.CustomerOrdersTotal"));
        $earn_points = $this->reports->filter_loyaltyreport($request);

       $myVar = new SiteSettingController();
        $result['setting'] = $myVar->getSetting();
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.reports.loyaltyreport", $title)->with('result', $result)->with(['earn_points'=> $earn_points, 'name'=> $request->FilterBy, 'param'=> $request->parameter]); 
    }

    public function loyaltyreportuser($id)
    {
        $title = array('pageTitle' => Lang::get("labels.CustomerOrdersTotal"));
        $user = $this->reports->loyaltyreportuser($id);
        $myVar = new SiteSettingController();
        $result['setting'] = $myVar->getSetting();
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.reports.loyaltyreport_user", $title)->with('result', $result)->with('user', $user);
    }

    public function couponreportuser($id)
    {
        $title = array('pageTitle' => Lang::get("labels.CustomerOrdersTotal"));
        $user = $this->reports->couponreportuser($id);
        $myVar = new SiteSettingController();
        $result['setting'] = $myVar->getSetting();
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.reports.couponreport_user", $title)->with('result', $result)->with('user', $user);

    }
    public function profit_loss(Request $request)
    {
        $title = array('pageTitle' => 'Profit Loss Report');
        if (isset($request->dateRange)) {
            $range = explode('-', $request->dateRange);
        ...
        $result['reports'] = $this->reports->ProfitLossReport($request);
        $result['currency'] = $this->Currency->getter();
        $result['price'] = $this->reports->customersReportTotal($request);
        $myVar = new SiteSettingController();
        $result['setting'] = $myVar->getSetting();
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.reports.profit_loss", $title)->with('result', $result);
    ...
    public function profitlossprint(Request $request)
    {
        ...
        return view("admin.reports.profit_loss_print", $title)->with('result', $result);
    }

    public static function get_profit_loss($date)
    {
        ...
        $report = DB::table('orders')
                  ...
                  ->whereDate('orders.date_purchased', '=', $dateFrom)
                  ->select('orders_products.*')
                  ->get();
          if(count($report)>0){
            foreach ($report as $jesreport) {
                $product = DB::table('products')->where('products_id', '=', $jesreport->products_id)->first();
                $amount =$jesreport->final_price;
                //$amount =$product->cost_price-$jesreport->final_price;
                if ($amount < 0){
                    $ltotal=$amount*$jesreport->products_quantity;
                    $loss=$loss+$ltotal;
                }else{
                   $total=$amount*$jesreport->products_quantity;
                   $profit=$profit+$total; 
                }
                
            }
          }
          $result = array('profit' => $profit,'loss'=>$loss);
        return  $result;
    }

    public function sales_person_report(Request $request)
    {
        $title = array('pageTitle' => 'Sales Person Report');
        if (isset($request->dateRange)) {
            $range = explode('-', $request->dateRange);
        ...
        $result['orders'] = $this->reports->sales_person_report($request);
        $result['salesman'] = $this->reports->get_sales_person();
        $myVar = new SiteSettingController();
        $result['setting'] = $myVar->getSetting();
        $result['commonContent'] = $this->Setting->commonContent();
        //print_r($result['dateFrom']);die();
        return view("admin.reports.sales_person_report", $title)->with('result', $result);
    }
    public function sales_person_view($id)
    {
        $commission  = DB::table('salesperson_commission')->where('order_id', '=', $id)->first();
        return view("admin.reports.sales_person_view")->with('result', $commission);
    }
    public function salescancelreport(Request $request)
    {
        $title = array('pageTitle' =>"Sales Cancel Report");
        if (isset($request->dateRange)) {
            $range = explode('-', $request->dateRange);
        ...
        $result['reports'] = $this->reports->salescancelreport($request);
        $result['price'] = $this->reports->customersReportTotal($request);
        ...
        return view("admin.reports.salescancelreport", $title)->with('result', $result);
    }
}
