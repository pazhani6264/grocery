...
class ShippingMethodsController extends Controller
{
    ...
    public function flaterate(Request $request)
    {
        ...
        $shipping_methods = $this->Shipping_method->shipingMethods($request->id);
        $result['flate_rate'] = $shipping_methods;


        $shipping_methods = $this->Shipping_method->shipingMethod4($request->id);
        $result['shipping_methods'] = $shipping_methods;

        //get function from other controller

        $result['languages'] = $this->Languages->englishgetter();

        $description_data = array();
        foreach ($result['languages'] as $languages_data) {

            $description = $this->Shipping_method->description($languages_data, $shipping_methods,$request->id);
    ...
    public function updateflaterate(Request $request)
    {
        ...
        //get function from other controller
        $languages = $this->Languages->englishgetter();
        foreach ($languages as $languages_data) {
            $name = 'name_' . $languages_data->languages_id;
            $content = array();

            $checkExist = $this->Shipping_method->CheckExit($table_name, $languages_data,$id);
            if (count($checkExist) > 0) {
                $languages_data_id = $languages_data->languages_id;
                $request_name = $request->$name;
               
                $this->Shipping_method->shipingDescription($table_name, $languages_data_id, $request_name,$id,$flate_rate,$currency);
            } else {
                $languages_data_id = $languages_data->languages_id;
                $request_name = $request->$name;

                $this->Shipping_method->insertDescription($request_name, $languages_data_id, $table_name,$flate_rate,$currency);
            }
        }
