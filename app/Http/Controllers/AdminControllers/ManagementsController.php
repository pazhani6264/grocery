...
class ManagementsController extends Controller
{
  ...
  public function factory_reset(Request $request){
    $title = array('pageTitle' => Lang::get("labels.factory_reset"));
    
    $result['commonContent'] = $this->Setting->commonContent();
    return view("admin.managements.factory_reset", $title)->with('result',$result);
  }
  ...
  public function reset_action(Request $request)
  {
      ...
      // collections 
        ...
        //DB::table('collections_description')->truncate();
        DB::table('collections_product')->truncate();
      //compare
        DB::table('compare')->truncate();
      //flash_sale
        DB::table('flash_sale')->truncate();
      //news
        DB::table('news')->truncate();
        DB::table('news_categories')->truncate();
        DB::table('news_categories_description')->truncate();
        DB::table('news_description')->truncate();
      // sliders_images 
        DB::table('sliders_images')->truncate();
      // categories
      ...
      //inventory
        ...
        DB::table('inventory_detail')->truncate();
      // order table
        DB::table('orders')->truncate();
        DB::table('orders_products')->truncate();
        DB::table('orders_products_attributes')->truncate();
        DB::table('orders_status_history')->truncate();
        DB::table('premier_pay')->truncate();
      // reviews table
      ...
      //user
        ...
        DB::table('customers_basket')->truncate();
        DB::table('customers_basket_attributes')->truncate();
        DB::table('customers_info')->truncate();
        DB::table('whos_online')->truncate();
      //coupons
        DB::table('coupons')->truncate();
        DB::table('redeem_points_description')->truncate();
        DB::table('redeem_points_settings')->truncate();
        DB::table('redeem_points_voucher')->truncate();
        DB::table('temp_point_transaction')->truncate();
        DB::table('transaction_points')->truncate();

         //tickets
         DB::table('tickets')->truncate();
        DB::table('tickets_products')->truncate();
         //client brand
         DB::table('clientbrand')->truncate();
        ...
        $customers_id = DB::table('users')->insertGetId([
            ...
            'last_name'         =>   'grocery',
            'phone'           =>   '',
            'email'           =>   'grocery@platinumcode.my',
            'password'          =>   '$2y$10$HW.RWivIXhSkpgAqVY4ov.wB4abs6Stzjg/GQKKLzxoBVEj2bZYi.',
            'status'          =>   '1',
            'avatar'          =>  '',
            'role_id'         =>   '1',
        ...
        // settings update
        DB::table("settings")->where('name', 'newsletter_image')->
        update(['value'     =>   'images/media/2020/11/OimK016812.png']);

        DB::table("settings")->where('name', 'website_logo')->
        update(['value'     =>   'images/media/2020/11/OimK016812.png']);

        DB::table("settings")->where('name', 'favicon')->
        update(['value'     =>   'images/media/2020/11/OimK016812.png']);

        DB::table("settings")->where('name', 'background_type')->
        update(['value'     =>   '1']);

        DB::table("settings")->where('name', 'background_color')->
        update(['value'     =>   '#FFFFFF']);

        DB::table("pos_payment_methods")->
        update(['image'     =>   '122']);

      // delete image
      $result = DB::table('images')->select('*')->where('id', '>',  '133')->get();
      if(!empty($result)){
        foreach ($result as $jesresult) {
          $image=DB::table('image_categories')->select('*')->where('image_id', '=', $jesresult->id)->get();
          if(!empty($image)){
             foreach ($image as $jesimage) {
               File::delete($jesimage->path);
               DB::table('image_categories')->where('id', $jesimage->id)->delete();
             }
          }
          DB::table('images')->where('id', $jesresult->id)->delete();
        }
      }

       return redirect()->back()->with('message', 'Factory reset successfully....');
  }
