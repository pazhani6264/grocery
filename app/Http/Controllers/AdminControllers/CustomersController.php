...
class CustomersController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        $customerData['errorMessage'] = $errorMessage;
        $result['commonContent'] = $this->Setting->commonContent();
        return view("admin.customers.add", $title)->with('customers', $customerData)->with('allimage',$allimage)->with('result', $result)->with('code', $countries);
    }
    ...
    public function insert(Request $request)
    {
        ...
        elseif (count($existPhone)> 0 ) {
          $messages = Lang::get("labels.Phone Number already exist");
          return Redirect::back()->withErrors($messages)->withInput($request->all());
      } 
    ...
    public function edit(Request $request){
      ...
      return view("admin.customers.edit",$title)->with('data', $customerData)->with('result', $result)->with('allimage', $allimage)->with('code', $countries);
    }
    ...
    public function delete(Request $request){
      $this->Customers->destroyrecords($request->users_id);
    ...
    public function view($id)
    {
      ...
      $customers = $this->Customers->edit($id);
      $customer_addresses = $this->Customers->addresses($id);
      $order=$this->Customers->paginator_order($id);
      return view("admin.customers.details",$title)->with('result', $result)->with(['customers' => $customers,'addresses'=> $customer_addresses,'order'=>$order]);
    }
