...
class NotificationController extends Controller
{
    ...
    public function notifyUser(Request $request)
    {
        ...
        if (count($devices) > 0) {
            ...
            $user = DB::table('users')->where('id', $devices[0]->user_id)->first();
            if($user !='')
            {
            $role =  DB::table('user_types')->where('user_types_id', $user->role_id)->first();
            $type = $role->user_types_name;
            }
            else
            {
                $type = 'Customers';
            }
            $response = $this->onesignalNotification($device_id, $sendData,  $type,$pageResponse);
            return $response;
        }
        else
        {
    ...
    public function sendNotifications(Request $request)
    {
        ...
        if ($device_type == 'all') { /* to all users notification */
            ...
            if (count($devices) > 0) {
                foreach ($devices as $devices_data) {
                    $type = 'Customers';
                    $response[] = $this->onesignalNotification($devices_data->device_id, $sendData, $type, $pageResponse);
        ...
        if ($message == 'sent') {
            return redirect()->back()->withErrors('Notification has been sent successfully');
    ...
    public function onesignalNotification($device_id, $sendData, $type, $pageResponse)
    {

        //get function from other controller
        //$settingss = $this->myVarSetting->getSetting();
        ...
        $settingss = $this->Setting->commonContent();
        if($type == 'Customers')
        {
        $auth_id = $settingss['setting']['onesignal_auth_id'];
        $app_id = $setting[55]->value;
        }
        if($type == 'Delivery Guy')
        {
        $auth_id = $settingss['setting']['deliveryboy_auth_id'];
        $app_id = $settingss['setting']['deliveryboy_app_id'];
        }
        if($type == 'Merchant')
        {
        $auth_id = $settingss['setting']['merchant_auth_id'];
        $app_id = $settingss['setting']['merchant_app_id'];
