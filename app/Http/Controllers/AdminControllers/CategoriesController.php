...
class CategoriesController extends Controller
{
  ...
  public function filter(Request $request){
    ...
    $result['commonContent'] = $this->Setting->commonContent();
    return view("admin.categories.index", $title)->with('result', $result)->with(['categories'=> $categories, 'name'=> $request->FilterBy, 'param'=> $request->parameter, 'param2'=> $request->parameter2]);
  }
  ...
  //  dd($editSubCategory[0]->parent_id);
   ...
   public function update(Request $request){
    ...
    public function deleteMultiple(Request $request){
      ...
      $deletecategory = $this->Categories->deleterecordMultiple($request);
      $message = Lang::get("labels.CategoriesDeleteMessage");
      return redirect()->back()->withErrors([$message]);
    }
    ...
    public function statusMultiple(Request $request){
      //print_r($request->category_id);die();
      $deletecategory = $this->Categories->statusMultiple($request);
      $message = Lang::get("labels.CategorieUpdateMessage");
      return redirect()->back()->withErrors([$message]);
    }
    ...
    public function cloneCategory(Request $request){
      //print_r($request->category_id);die();
      $deletecategory = $this->Categories->cloneCategory($request);
      $message = Lang::get("labels.CategorieUpdateMessage");
      return redirect()->back()->withErrors([$message]);
    }
    ...
    public function sorting(Request $request){
      $title = array('pageTitle' => Lang::get("labels.SubCategories"));
      $categories = $this->Categories->cateall($request);
      $result['commonContent'] = $this->Setting->commonContent();
      return view("admin.categories.sorting", $title)->with('result', $result)->with(['categories'=> $categories, 'name'=> $request->FilterBy, 'param'=> $request->parameter, 'param2'=> $request->parameter2]);
    }
	...
	public function updateorder(Request $request)
	{
      ...
      DB::table('categories')->where('categories_id', $id)->update(
        [
            'CategoryOrder'      =>   $categoryindex[$key]+1,
        ]);
  ...
  public function subcategorySorting(Request $request,$id){
    $title = array('pageTitle' => Lang::get("labels.SubCategories"));
    $categories = $this->Categories->subcateall($id);
    $result['commonContent'] = $this->Setting->commonContent();
    return view("admin.categories.subcategory_sorting", $title)->with('result', $result)->with(['categories'=> $categories, 'name'=> $request->FilterBy, 'param'=> $request->parameter, 'param2'=> $request->parameter2]);
  }


  public function updateordersubcat(Request $request)
	{
		 $categoryindex = $request->categoryindex;
		 $categoryid = $request->categoryid;
		 foreach($categoryid as $key=>$id)
