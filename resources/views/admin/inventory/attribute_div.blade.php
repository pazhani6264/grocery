    ...
    <div class="col-sm-6 col-md-6">
        ...
        <ul class="list-group list-group-root well list-group-root2">
            ...
            <li href="#" class="list-group-item"><label style="width:100%">
                    <input id="attribute_id" type="hidden" class="attributeid_<?=$attribute['option']['id']?>_<?=$result['products'][0]->products_id?>" name="attributeid<?=$result['products'][0]->products_id?>[]" value="0"> {{ $attribute['option']['name']}}</label></li>
            <ul class="list-group">
                <li class="list-group-item">
                    @foreach ($attribute['values'] as $value)<label><input name="values_<?=$attribute['option']['id']?>_<?=$result['products'][0]->products_id?>[]" type="radio" onClick="currentstockGet('<?=$result['products'][0]->products_id?>','{{ $value['products_attributes_id'] }}','{{ $attribute['option']['id'] }}')" class="required_one" value="{{ $value['products_attributes_id'] }}"
...
@elseif($result['products'][0]->products_type == '0')
    <span class="help-block" style="font-weight: normal;font-size: 11px;margin-bottom: 0;margin-top: -10px;">{{ trans('labels.Now you can add stock for simple product') }}</span>
@endif
