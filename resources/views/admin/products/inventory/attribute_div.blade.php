        ...
        <input type="hidden" name="attributes_id_new" class="attributes_id_new" value="">
            ...
            <ul class="list-group">
                <li class="list-group-item">
                    ...
                    <input type="hidden" value="{{ $value['products_attributes_id'] }}" class="attributes_id var-{{$value['id']}} new-{{ $attribute['option']['id'] }}">
                        <label>
                            @if($attribute['option']['options_select_type'] == 0)
                                <input name="values_<?=$attribute['option']['id']?>" type="radio" class=" required_one" value="{{ $value['products_attributes_id'] }}" attributeid="{{ $attribute['option']['id'] }}" onchange="updateActiveClass(event, {{$value['id']}}, {{ $value['products_attributes_id'] }} , 'radio',{{ $attribute['option']['id'] }},{{ $attribute['option']['id'] }})"> {{ $value['value'] }}
                            @else
                                <input name="values_<?=$attribute['option']['id']?>" type="checkbox" class=" required_one" value="{{ $value['products_attributes_id'] }}" attributeid="{{ $attribute['option']['id'] }}" onchange="updateActiveClass(event, {{$value['id']}}, {{ $value['products_attributes_id'] }} , 'checkbox', {{ $attribute['option']['id'] }}),{{ $attribute['option']['id'] }}"> {{ $value['value'] }}
                            @endif
                        </label> 
                    @endforeach
                </li>  
