   ...
   // dd($categories);
    foreach($categories as $parents){
     ...
     if(isset($parents->childs)){
        ...
        $product_count = "";
        $hasChild1 = url('shop?category=').$parents->slug;
      }else {
        $hasChildicon = "";
        $product_count = '<span class="p24-count-span">'.$product_count.'</span>';
      ...
      $option .= '<li>
      <div class="link"><a href='.$hasChild1.'>'.$parents->categories_name.'</a><a href='.$hasChild.'>'.$hasChildicon.'</a>'.$product_count.'</div>';
 ...
 function shopChildcat($childs, $i, $parent_id){
  ...
  foreach($childs as $key => $child){
    ...
    $contents.= '<li>
        ...
        '.$child->categories_name.'  <span class="p24-count-span">'.$product_count_child.'</span>
        </a>
      </li>';
