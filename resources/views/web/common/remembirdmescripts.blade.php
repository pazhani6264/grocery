...
jQuery(document).on('click', '#subsave', function(e){
  ...
  if(login == ''){
    ...
    var appTime = $("#app_time:checked").val();
    ...
    //alert(products_id);
    if(outlet == ''){
        $('.err-req0').html('Outlet is required');
    } else if(name == ''){
      ...
      jQuery.ajax({
        ...
        type: "POST",
        data: '&outlet_id='+outlet+'&products_id='+products_id+'&product_name='+product_name+'&name='+name+'&phone='+phone+'&appDate='+appDate+'&appTime='+appTime+'&message='+message+'&address='+address,
        success: function (res) {  
...
$(document).ready(function () {
<?php  $allholidays=DB::table('appointment_holiday')->where('status',1)->get();  ?>

var holiday = [];

@foreach ($allholidays as $key=>$holidays)

var test =  "{{date('d-n-Y', strtotime($holidays->date))}}";
