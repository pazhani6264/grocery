...
<div class="product-molla ajax_product_42 product-molla-42 product product9 product4 box-shadow" style="background-color:{{ $result['commonContent']['settings']['card_background'] }}">
  <article>
  @if($result['commonContent']['settings']['product_column'] == 1)
     ...
     <?php if(auth()->guard('customer')->check() &&  auth()->guard('customer')->user()->phone_verified== '1')
                  {
                    ...
                    { ?>
                      <a id="wish_molla_show_{{$products->products_id}}" class="wish_molla_show wishlist-style-42 btn-expandable  active is_liked_molla_1" products_id="<?= $products->products_id ?>"><spans>add to wishlist</spans><i class="fa fa-heart-o"></i></a>

                      <a style="display:none;" id="wish_molla_hide_{{$products->products_id}}" class="wish_molla_hide wishlist-style-42 btn-expandable  active "  href="{{url('wishlist')}}"><spans>go to wishlist</spans><i class="fa fa-heart"></i></a>
                <?php } else { ?>
                      <a class="wishlist-style-42 btn-expandable  active" href="{{url('wishlist')}}"><spans>go to wishlist</spans><i class="fa fa-heart"></i></a>
                <?php } } else { ?>
                      <a class="wishlist-style-42 btn-expandable  active is_liked_molla_1" products_id="<?= $products->products_id ?>"><spans>add to wishlist</spans><i class="fa fa-heart-o"></i></a>
                <?php } ?>

       <!--  <a class="wishlist-style-42 btn-expandable active is_liked" products_id="<?= $products->products_id ?>" data-toggle="" data-placement="bottom" title="@lang('website.Wishlist')">
        <spans>add to wishlist</spans>
        <i class="fa fa-heart-o"></i>
      </a> -->

      <div class="icon border-radius-50 wishlist-42 modal_show2 cursor-pointer"  products_id="{{$products->products_id}}" data-toggle="" data-placement="bottom" title="Quick View"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 39.356 27.497">
     ...
     <div class="product-action">
          ...
          <div class="btns btn-blocks swipe-to-top padding-0">
          <?php
          ...
          @if($products->button_type == 1 || $products->button_type == 3)
                      ...
                      @if(!in_array($products->products_id,$result['cartArray']))
                        ...
                        @if($products->stock_status == 1)
                          @if($products->defaultStock<=0)
                            <button type="button" class="btn btn-blocks  btn-42-danger swipe-to-top" products_id="{{$products->products_id}}" >@lang('website.Out of Stock')</button>
                          @else
                              <button type="button" class="btn btn-blocks  btn-42 btn-secondary cart swipe-to-top" products_id="{{$products->products_id}}" >@lang('website.Add to Cart')</button>
                          @endif
                          @else
                              <button type="button" class="btn btn-blocks  btn-42 btn-secondary cart swipe-to-top" products_id="{{$products->products_id}}" >@lang('website.Add to Cart')</button>
                          @endif
                        ...
                        @endif
                      @else
                          <button type="button" class="btn btn-42 btn-secondary active swipe-to-top" >@lang('website.Added')</button>
                      @endif
                  @elseif($products->products_type==1)
                      <a class="btn btn-42 btn-secondary swipe-to-top" href="{{ URL::to('/product-detail/'.$products->products_slug)}}" >@lang('website.View Detail')</a>
                  @elseif($products->products_type==2)
                      <a href="{{$products->products_url}}" target="_blank" class="btn btn-42 btn-secondary  swipe-to-top" >@lang('website.External Link')</a>
                  ...
                  <button type="button"  class="btn btn-blocks  btn-42 btn-secondary  swipe-to-top modal_show3" products_id="{{$products->products_id}}" products_name="{{$products->products_name}}">Book</button>
...
<?php 
     ...
     <div class="pro-rating" style="margin-top:10px">
                <fieldset class="disabled-ratings-19 text-center">                                           
                  ...
                  <a style="font-size:0.8rem" href="#review" id="review-tabs" data-toggle="pill" role="tab" class="btn-link mobile-review-center tab-land-review-33">( {{$products->total_user_rated}} @lang('website.Reviews') )</a>
