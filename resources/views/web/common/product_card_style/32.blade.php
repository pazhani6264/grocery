...
<div class="product-molla ajax_product_32 product-molla-32 product product9 product4" style="background-color:{{ $result['commonContent']['settings']['card_background'] }}">
  <article>
  @if($result['commonContent']['settings']['product_column'] == 1)
     ...
     <?php if(auth()->guard('customer')->check() &&  auth()->guard('customer')->user()->phone_verified== '1')
        ...
        { ?>
          <a id="wish_molla_show_{{$products->products_id}}" class="wish_molla_show border-radius-50 cursor-pointer wishlist-32 active swipe-to-top is_liked_molla_1" products_id="<?= $products->products_id ?>" data-toggle="" data-placement="bottom" title="add to wishlist"><i class="fa fa-heart-o heart_show "></i><i style="display:none;" class="fa fa-heart  heart_hide"></i></a>

          <a style="display:none;" id="wish_molla_hide_{{$products->products_id}}" class="wish_molla_hide border-radius-50 cursor-pointer wishlist-32 active swipe-to-top"  href="{{url('wishlist')}}" data-toggle="" data-placement="bottom" title="go to wishlist"><i class="fa fa-heart "></i></a>
    <?php } else { ?>
          <a class="border-radius-50 cursor-pointer wishlist-32 active swipe-to-top" href="{{url('wishlist')}}"  data-toggle="" data-placement="bottom" title="go to wishlist"><i class="fa fa-heart "></i></a>
    <?php } } else { ?>
          <a class="border-radius-50 cursor-pointer wishlist-32 active swipe-to-top is_liked_molla_1" products_id="<?= $products->products_id ?>" data-toggle="" data-placement="bottom" title="add to wishlist"><i class="fa fa-heart-o "></i></a>
    <?php } ?>

       <!--  <a class="border-radius-50 wishlist-32 active swipe-to-top is_liked" products_id="<?= $products->products_id ?>" data-toggle="" data-placement="bottom" title="@lang('website.Wishlist')"><i class="fas fa-heart"></i></a> -->

        <div class="border-radius-50 wishlist-32 swipe-to-top modal_show2 cursor-pointer quick-fill common-fill-hover"  products_id="{{$products->products_id}}" data-toggle="" data-placement="bottom" title="Quick View"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 39.356 27.497">
     ...
     <?php
        ...
        @if($products->button_type == 1 || $products->button_type == 3)
                      ...
                      @if(!in_array($products->products_id,$result['cartArray']))
                        ...
                        @if($products->stock_status == 1)
                          @if($products->defaultStock<=0)
                            <button type="button" class="btn btn-blocks  btn-32-dagner swipe-to-top" products_id="{{$products->products_id}}" > @lang('website.Out of Stock')</button>
                          @else
                              <button type="button" class="btn btn-blocks quick-fill common-fill-hover btn-32 cart swipe-to-top" products_id="{{$products->products_id}}" ><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 50.948 44.099" style="margin-right:20px;">
                        ...
                        </svg></i>@lang('website.Add to Cart')</button>
                          @endif
                          @else
                              <button type="button" class="btn btn-blocks quick-fill common-fill-hover btn-32 cart swipe-to-top" products_id="{{$products->products_id}}" ><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 50.948 44.099" style="margin-right:20px;">
                          <path id="cart_plus" d="M30.728,37.537a6.564,6.564,0,0,1,4.949-6.361H19.219a6.561,6.561,0,1,1-3.226,0H13.216l-.4-1.44L5.589,3.938H0V0H8.575l.4,1.439,7.224,25.8H39.28L46.6,9.93h4.345L41.889,31.176H38.9a6.562,6.562,0,1,1-8.175,6.361Zm3.281,0a3.281,3.281,0,1,0,3.28-3.28A3.284,3.284,0,0,0,34.009,37.537Zm-19.684,0a3.281,3.281,0,1,0,3.281-3.28A3.284,3.284,0,0,0,14.325,37.537Zm12.659-18.3v-6.76H20.19V8.919h6.794V2.125h3.547V8.919H37.29v3.563H30.531v6.76Z"/>
                        ...
                        @endif
                      @else
                          <button type="button" class="btn btn-32 active swipe-to-top" >@lang('website.Added')</button>
                      @endif
                  @elseif($products->products_type==1)
                      <a class="btn btn-32 swipe-to-top" href="{{ URL::to('/product-detail/'.$products->products_slug)}}" >@lang('website.View Detail')</a>
                  @elseif($products->products_type==2)
                      <a href="{{$products->products_url}}" target="_blank" class="btn btn-32  swipe-to-top" >@lang('website.External Link')</a>
              ...
              @elseif($products->button_type == 2)
                <button type="button" class="btn btn-blocks  btn-32 swipe-to-top modal_show3" products_id="{{$products->products_id}}" products_name="{{$products->products_name}}">Book</button>
