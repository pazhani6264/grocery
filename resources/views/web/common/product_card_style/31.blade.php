...
<div class="product-molla ajax_product_31 product-molla-31 product box-shadow product9 product4" style="background-color:{{ $result['commonContent']['settings']['card_background'] }}">
  <article>
     ...
     <?php if(auth()->guard('customer')->check() &&  auth()->guard('customer')->user()->phone_verified== '1')
                  {
                    ...
                    { ?>
                      <a id="wish_molla_show_{{$products->products_id}}" class="wish_molla_show wishlist-style-31 btn-expandable  active is_liked_molla_1" products_id="<?= $products->products_id ?>"><spans>add to wishlist</spans><i class="fa fa-heart-o"></i></a>

                      <a style="display:none;" id="wish_molla_hide_{{$products->products_id}}" class="wish_molla_hide wishlist-style-31 btn-expandable  active "  href="{{url('wishlist')}}"><spans>go to wishlist</spans><i class="fa fa-heart"></i></a>
                <?php } else { ?>
                      <a class="wishlist-style-31 btn-expandable  active" href="{{url('wishlist')}}"><spans>go to wishlist</spans><i class="fa fa-heart"></i></a>
                <?php } } else { ?>
                      <a class="wishlist-style-31 btn-expandable  active is_liked_molla_1" products_id="<?= $products->products_id ?>"><spans>add to wishlist</spans><i class="fa fa-heart-o"></i></a>
                <?php } ?>

   <!--   <a class="wishlist-style-31 btn-expandable active  is_liked" products_id="<?= $products->products_id ?>" data-toggle="" data-placement="bottom" title="@lang('website.Wishlist')">
     ...
     <div class="product-action" style="margin:0px 10px">
     <?php
           ...
           @if($products->button_type == 1 || $products->button_type == 3)
                @if($products->products_type==0)
                      @if(!in_array($products->products_id,$result['cartArray']))
                        ...
                        @if($products->stock_status == 1)
                          @if($products->defaultStock<=0)
                            <button style="display:none" type="button" class="btn btn-blocks  btn-31-dagner swipe-to-top" products_id="{{$products->products_id}}" >@lang('website.Out of Stock')</button>
                          @else
                              <button type="button" class="btn btn-blocks  btn-31 cart swipe-to-top" products_id="{{$products->products_id}}" ><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 50.948 44.099">
                       ...
                       <path id="cart_plus" d="M30.728,37.537a6.564,6.564,0,0,1,4.949-6.361H19.219a6.561,6.561,0,1,1-3.226,0H13.216l-.4-1.44L5.589,3.938H0V0H8.575l.4,1.439,7.224,25.8H39.28L46.6,9.93h4.345L41.889,31.176H38.9a6.562,6.562,0,1,1-8.175,6.361Zm3.281,0a3.281,3.281,0,1,0,3.28-3.28A3.284,3.284,0,0,0,34.009,37.537Zm-19.684,0a3.281,3.281,0,1,0,3.281-3.28A3.284,3.284,0,0,0,14.325,37.537Zm12.659-18.3v-6.76H20.19V8.919h6.794V2.125h3.547V8.919H37.29v3.563H30.531v6.76Z" fill="#fff"/>
                        ...
                        @endif
                      @else
                          <button type="button" class="btn btn-blocks btn-31 active swipe-to-top" >@lang('website.Added')</button>
                      @endif
                  @elseif($products->products_type==1)
                      <a class="btn btn-blocks btn-31 swipe-to-top" href="{{ URL::to('/product-detail/'.$products->products_slug)}}" >&nbsp;&nbsp; @lang('website.View Detail')</a>
                  @elseif($products->products_type==2)
                      <a href="{{$products->products_url}}" target="_blank" class="btn btn-blocks btn-31  swipe-to-top" >&nbsp;&nbsp; @lang('website.External Link')</a>
            ...
            <button type="button"  class="btn btn-block btn-31 swipe-to-top modal_show3" products_id="{{$products->products_id}}" products_name="{{$products->products_name}}">Book</button>
