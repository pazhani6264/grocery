<?php
 ...
 function productCategories(){
  ...
  $option .= '<b><a class="dropdown-item categories-list demo-13-cat common-hover" value="'. Lang::get("website.Choose Any Category") .'">'. Lang::get("website.Choose Any Category") .'</a></b>';
    foreach($categories as $parents){
      ...
      $option .= '<b><a class="dropdown-item categories-list demo-13-cat common-hover '.$selected.'" value="'.$parents->categories_name.'" slug="'.$parents->slug.'" '.$selected.'>'.$parents->categories_name.'</a></b>';
 ...
 function childcat($childs, $i, $parent_id){
  ...
  foreach($childs as $key => $child){
    ...
    $contents.='<a class="dropdown-item categories-list demo-13-cat common-hover '.$selected.'"  value="'.$child->categories_name.'" slug="'.$child->slug.'" '.$selected.'>'.$dash.$child->categories_name.'</a>';
