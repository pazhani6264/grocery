...
<?php
   ...
   // dd($categories);
    foreach($categories as $parents){
     ...
     if(isset($parents->childs)){
      ...
      $option .= '
       <a style="display:block;background-color:#fff !important;color:#333" class=" main-manu cate-11-bottom-border"'. $hasChild .'>'.$parents->categories_name.'</a>';
 ...
 function childcatmobile11($childs, $i, $parent_id){
  ...
  foreach($childs as $key => $child){
    ...
    $contents.= '<li style="padding:10px 0px" class="list-item">
        <a class="list-link" href='.url('shop?category=').$child->slug.' > '.$dash.''.$child->categories_name.'
        </a>
      </li>';
