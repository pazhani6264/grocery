...
<div class="page__title ticket-common-bg">
	<h1>{{ __('Welcome back '.auth()->guard('customer')->user()->first_name).' !'}}</h1>
</div>
					...
					<div class="card edit-card">
						<div class="card-header">
							...
							<span class="float-right">
								<a href="{{url('/view_tickets#opened')}}" class="btn btn-outline-primary btn-sm">{{ __('View all')}}</a>
							</span>
						</div>
						<div class="card-body">
				...
				<div class="col-lg-4">
					<div class="card edit-card">
						<div class="card-header">
							...
							{{__('Last Answered Tickets')}} <span class="float-right"><a href="{{url('/view_tickets#answered')}}" class="btn btn-outline-primary btn-sm">{{ __('View all')}}</a></span></div>
							<div class="card-body">
				...
				<div class="col-lg-4">
					<div class="card edit-card">
						<div class="card-header">
							...
							{{__('Last Closed Tickets')}} <span class="float-right"><a href="{{url('/view_tickets#closed')}}" class="btn btn-outline-primary btn-sm">{{ __('View all')}}</a></span></div>
							<div class="card-body">
							@if($closed_tickets->count())
          @foreach($closed_tickets as $closed_ticket)
								<div class="ticket" onclick="window.location.href='{{url('ticket')}}/{{$closed_ticket->id}}'">
									<div class="row">
										<div class="col-lg-8">
