...
<div id="getparallexbannertwo_loading" ></div>

  <div id="getparallexbannertwo_product" class="@if($current_theme->template == 0) common-padding-topbottom-{{$margin_between->value}} @endif" style="@if($current_theme->template != 0) margin-top:30px; @endif"></div>

  <script>
    getparallexbanner_two();
    function getparallexbanner_two() {
      var type = 'two'
      var content ='';
      ...
      jQuery.ajax({
          url: '{{ URL::to("/getparallexbanner_two")}}',
          headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
          type: "POST",
          data: 'type='+type,
            success: function (res) { 
              jQuery('#getparallexbannertwo_loading').html(' ');
              jQuery('#getparallexbannertwo_product').html(res);
