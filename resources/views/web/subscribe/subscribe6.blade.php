<style>
.subfill{
    fill:#333;
}
.subfill:hover{
    fill:#fff;
}
.container-sub4 {
    width: 1195px !important;
    max-width: 100%;
}

.demo-5-focus:active
{
  border:solid 1px  #9ACE69 !important;
}
.cta-border-image {
    background-size: cover;
    background-repeat: no-repeat;
    border: none;
    padding: 0.75rem;
}
.mb-0 {
    margin-bottom: 0!important;
}
.cta-separator {
    position: relative;
}
.cta-border-wrapper {
    padding-top: 0.9rem;
    padding-bottom: 3rem;
}
.css-lmt9ai {
    animation-duration: 1000ms;
    animation-timing-function: ease;
    animation-delay: 200ms;
    animation-name: animation-1fopo5u;
    animation-direction: normal;
    animation-fill-mode: both;
    animation-iteration-count: 1;
}
.cta-half .cta-wrapper.cta-text {
    max-width: 380px;
}
.cta-wrapper {
    padding-top: 3.5rem;
    padding-bottom: 2rem;
    max-width: 370px;
    margin-left: auto;
    margin-right: auto;
}
.cta-half .cta-wrapper.cta-text .cta-title, .cta-half .cta-wrapper .cta-title {
    margin-bottom: 0;
}
.cta-half .cta-title {
    font-weight: 600;
    font-size: 2.15rem;
    letter-spacing: -.025em;
    margin-bottom: 0;
}
.cta-half .cta-desc {
    color: #999;
    font-size: 1.15rem;
    line-height: 1.5;
    font-weight:300 !important;
}
 .cta-desc {
    margin-bottom: 1.5rem;
}
.social-icons-colored .social-icon.social-facebook {
    color: #fff;
    border-color: #8f79ed;
    background-color: #8f79ed;
}
.social-icons-colored .social-icon.social-twitter {
    color: #fff;
    border-color: #79c8ed;
    background-color: #79c8ed;
}
.social-icons-colored .social-icon.social-instagram {
    color: #fff;
    border-color: #dd6d9a;
    background-color: #dd6d9a;
}
.social-icons-colored .social-icon.social-pinterest, .social-icons-colored .social-icon.social-youtube {
    color: #fff;
    border-color: #e66262;
    background-color: #e66262;
}
.social-icons-colored .social-icon.social-pinterest, .social-icons-colored .social-icon.social-youtube {
    color: #fff;
    border-color: #e66262;
    background-color: #e66262;
}

.social-icons-colored .social-icon:focus.social-facebook, .social-icons-colored .social-icon:hover.social-facebook {
    color: #8f79ed;
}
.social-icons-colored .social-icon:focus.social-twitter, .social-icons-colored .social-icon:hover.social-twitter {
    color: #79c8ed;
}
.social-icons-colored .social-icon:focus.social-instagram, .social-icons-colored .social-icon:hover.social-instagram {
    color: #dd6d9a;
}
.social-icons-colored .social-icon:focus.social-youtube, .social-icons-colored .social-icon:hover.social-youtube {
    color: #e66262;
}
.social-icons-colored .social-icon:focus.social-pinterest, .social-icons-colored .social-icon:hover.social-pinterest {
    color: #e66262;
}
.social-icons-colored .social-icon:focus, .social-icons-colored .social-icon:hover {
    background-color: #fff!important;
    border-color: #d7d7d7!important;
}

.cta-half .social-icon {
    width: 3.5rem;
    height: 3.5rem;
    font-size: 1.3rem;
}
.social-icon:not(:last-child) {
    margin-right: 1rem;
}
.social-icon {
    justify-content: center;
    font-size: 1.6rem;
    width: 4rem;
    height: 4rem;
    color: #777;
    margin: 0;
    background-color: transparent;
    border: 0.1rem solid #e1e2e6;
    border-radius: 50%;
    text-decoration: none;
    opacity: 1;
    -webkit-transition: all .35s ease;
    transition: all .35s ease;
}
.social-icon, .social-icons {
    display: flex;
    align-items: center;
}
.cta-separator [class*=col-]+[class*=col-]:before {
    content: "";
    width: 0.1rem;
    display: block;
    height: 80%;
    position: absolute;
    left: 0;
    top: 50%;
    -webkit-transform: translateY(-50%);
    transform: translateY(-50%);
    -ms-transform: translateY(-50%);
    background-color: #ebebeb;
}
.css-mxanrf {
    animation-duration: 1000ms;
    animation-timing-function: ease;
    animation-delay: 200ms;
    animation-name: animation-qlkiml;
    animation-direction: normal;
    animation-fill-mode: both;
    animation-iteration-count: 1;
}

.input-group {
    position: relative;
    display: -ms-flexbox;
    display: flex;
    -ms-flex-wrap: wrap;
    flex-wrap: wrap;
    -ms-flex-align: stretch;
    align-items: stretch;
    width: 100%;
}
.cta .form-control:not(:focus):not(.form-control-white) {
    border-color: #dadada;
}
.cta-half .input-group .form-control {
    border-radius:30px !important;
    position:relative;
}
.demo-5-btn-new
{
    position:absolute;
    right:0;
}

.cta .form-control {
    height: 46px;
    padding-top: 1.15rem;
    padding-bottom: 1.15rem;
    background-color: #fff;
}
.input-group>.custom-file, .input-group>.custom-select, .input-group>.form-control, .input-group>.form-control-plaintext {
    position: relative;
    -ms-flex: 1 1 auto;
    flex: 1 1 auto;
    width: 1%;
    margin-bottom: 0;
}
.css-mxanrf .form-control {
    height: 46px;
    padding: 0.85rem 2rem;
    font-size: 1rem;
    line-height: 1.5;
    font-weight: 300;
    color: #777;
    background-color: #fff;
    border: 1px solid #ebebeb !important;
    border-radius: 0;
    margin-bottom: 2rem;
    -webkit-transition: all .3s;
    transition: all .3s;
    outline-width: 0;
}
.input-group-append {
    margin-left: -1px;
}
.cta-half .input-group .btn {
    min-width: 70px !important;
    border-top-right-radius: 30px !important;
    border-bottom-right-radius: 30px !important;
    height:46px;
}

@media screen and (max-width: 767px){
    .cta-wrapper {
        padding-left: 1rem;
        padding-right: 1rem;
    }
    .container-sub4 {
        padding-left:10px !important;
        padding-right:10px !important;
    }
    .css-mxanrf .btn:not(.btn-block) {
        max-width: 300px;
        padding: 0;
        vertical-align: text-bottom;
        text-transform: initial;
        display: block;
        margin: 0 !important;
        margin: auto;
    }
}

@media screen and (max-width: 991px){
    .cta-separator [class*=col-]+[class*=col-]:before {
        height: 0.1rem;
        width: 80%;
        left: 10%;
        top: 0;
        bottom: auto;
    }
    .cta-wrapper {
        padding-top: 3.1rem;
        padding-bottom: 3.1rem;
        max-width: 370px;
        margin-left: auto;
        margin-right: auto;
    }
}

</style>
<?php   $subscribe = DB::table('home_subscribe')
        ->leftJoin('image_categories', 'home_subscribe.subscribe_image_id', 'image_categories.image_id')
        ->select('home_subscribe.*', 'image_categories.path_type as image_path_type', 'image_categories.path as image_path')
        ->where('image_type', 'ACTUAL')
        ->first(); if($subscribe !='') {?>

<div class="container container-sub4">
    <div class="cta cta-separator cta-border-image cta-half mb-0">
        <div class="cta-border-wrapper bg-white">
            <div class="row">
                <div class="col-lg-6">
                    <div class="css-lmt9ai">
                        <div class="cta-wrapper cta-text text-center">
                            <h3 class="cta-title">Shop Social</h3>
                            <p class="cta-desc">Donec nec justo eget felis facilisis fermentum. Aliquam porttitor mauris sit amet orci. </p>
                            <div class="social-icons social-icons-colored justify-content-center">
                                <a class="social-icon social-facebook" href="#"><i class="fa fa-facebook"></i></a>
                                <a class="social-icon social-twitter" href="#"><i class="fa fa-twitter"></i></a>
                                <a class="social-icon social-instagram" href="#"><i class="fa fa-instagram"></i></a>
                                <a class="social-icon social-youtube" href="#"><i class="fa fa-youtube"></i></a>
                                <a class="social-icon social-pinterest" href="#"><i class="fa fa-pinterest"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="css-mxanrf">
                        <div class="cta-wrapper text-center">
                            <h3 class="cta-title">{{ $subscribe->subscribe_title }}</h3>
                            <p class="cta-desc">and <br>{{ $subscribe->subscribe_description }}</p>
                            <form action="{{url('subscribeMail')}}">
                                <div class="input-group">
                                    <input type="email" class="form-control demo-5-focus" placeholder="Enter your Email Address" aria-label="Email Adress" required="">
                                    <div class="input-group-append demo-5-btn-new">
                                        <button id="mc-embedded-subscribe" style="border-left-width: 0;" class="btn btn-rounded sub6-common-hover demo-5-focus subfill" type="submit"><svg xmlns="http://www.w3.org/2000/svg" width="11" height="17" viewBox="0 0 33.908 19.619" style="margin-top: -5px;">
             <path id="arrow_right" d="M28.861,11.627a1.335,1.335,0,0,0-.01,1.88l6.212,6.223L6.928,19.342a1.328,1.328,0,0,0,0,2.657l28.125.388-6.212,6.223a1.345,1.345,0,0,0,.01,1.88,1.323,1.323,0,0,0,1.87-.01L39.14,22h0a1.492,1.492,0,0,0,.276-.419,1.268,1.268,0,0,0,.1-.511,1.332,1.332,0,0,0-.378-.93l-8.42-8.481A1.3,1.3,0,0,0,28.861,11.627Z" transform="translate(-5.61 -11.252)"/>
           </svg></button>
                                    </div>
                                    <div class="alert alert-success alert-dismissible success-subscribte" role="alert" style="opacity: 500; display: none;"></div>
                                    <div class="alert alert-danger alert-dismissible error-subscribte" role="alert" style="opacity: 500; display: none;"></div>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<?php  } ?>

