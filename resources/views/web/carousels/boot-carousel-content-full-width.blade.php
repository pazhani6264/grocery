...
<div id="getcarousel_2_loading"></div>

  <div id="getcarousel_2_product" class="@if($current_theme->template == 0) common-padding-bottom-{{$bottom}} @endif" style="@if($current_theme->template != 0) padding-top:30px; @endif"></div>

  <script>
    getcarousel_2();
    function getcarousel_2() {
      var type = '2'
      var content ='';
      ...
      jQuery('#getcarousel_2_loading').html(content);
      
      jQuery.ajax({
          url: '{{ URL::to("/getcarousel_2")}}',
          headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
          type: "POST",
          data: 'type='+type,
            success: function (res) { 
              jQuery('#getcarousel_2_loading').html(' ');
              jQuery('#getcarousel_2_product').html(res);
