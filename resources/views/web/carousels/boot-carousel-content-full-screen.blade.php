...
<div id="getcarousel_1_loading"></div>

  <div id="getcarousel_1_product" class="@if($current_theme->template == 0) common-padding-bottom-{{$bottom}} @endif"></div>

  <script>
    getcarousel_1();
    function getcarousel_1() {
      var type = '1'
      var content ='';
      ...
      jQuery('#getcarousel_1_loading').html(content);
      
      jQuery.ajax({
          url: '{{ URL::to("/getcarousel_1")}}',
          headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
          type: "POST",
          data: 'type='+type,
            success: function (res) { 
              jQuery('#getcarousel_1_loading').html(' ');
              jQuery('#getcarousel_1_product').html(res);
