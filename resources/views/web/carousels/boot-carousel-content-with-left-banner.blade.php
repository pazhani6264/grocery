...
<div id="getcarousel_3_loading"></div>

  <div id="getcarousel_3_product" class="@if($current_theme->template == 0) common-padding-bottom-{{$bottom}} @endif"></div>

  <script>
    getcarousel_3();
    function getcarousel_3() {
      var type = '3'
      var content ='';
      ...
      jQuery('#getcarousel_3_loading').html(content);
      
      jQuery.ajax({
          url: '{{ URL::to("/getcarousel_3")}}',
          headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
          type: "POST",
          data: 'type='+type,
            success: function (res) { 
              jQuery('#getcarousel_3_loading').html(' ');
              jQuery('#getcarousel_3_product').html(res);
