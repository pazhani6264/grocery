...
.gettrend6  .slick-slide {
    outline: none;
    padding: 0 10px;
}

   .gettrend6{
    padding:2rem 0rem 3.5rem 0;
   }
...
.demo-20-top-4-section .product-molla .padd-10 {
...
.product-molla-33 article .thumb{
  margin: 30px 10px 0 10px;
   ...
   <div id="gettrend10_section_loading" ></div>
   
     <div id="gettrend10_section_product"></div>
   
     <script>
       getalltrendproduct10_section();
       function getalltrendproduct10_section() {
         var type = '2'
         var content ='';
         ...
         jQuery('#gettrend10_section_loading').html(content);
         
         jQuery.ajax({
             url: '{{ URL::to("/getalltrendproduct10_section")}}',
             headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
             type: "POST",
             data: 'type='+type,
               success: function (res) { 
                 jQuery('#gettrend10_section_loading').html(' ');
                 jQuery('#gettrend10_section_product').html(res);
   ...
   jQuery(document).ready(function () {
    (function (jQuery) {
    var tabCarousel = jQuery('.trendpro-carousel-js9');
