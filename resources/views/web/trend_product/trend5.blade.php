...
<div id="gettrend5_section_loading" ></div>

  <div id="gettrend5_section_product"></div>

  <script>
    getalltrendproduct5_section();
    function getalltrendproduct5_section() {
      var type = '2'
      var content ='';
      ...
      jQuery('#gettrend5_section_loading').html(content);
      
      jQuery.ajax({
          url: '{{ URL::to("/getalltrendproduct5_section")}}',
          headers: {'X-CSRF-TOKEN': jQuery('meta[name="csrf-token"]').attr('content')},
          type: "POST",
          data: 'type='+type,
            success: function (res) { 
              jQuery('#gettrend5_section_loading').html(' ');
              jQuery('#gettrend5_section_product').html(res);
...
$(document).ready(function() {
  ...
  $('.p24-slider-cat').hide();
  $('#p24-slider-cat-trend5-0').show();

  $('.p24-tabs-nav').on('click', function (event) {
    event.preventDefault();
    
    $('.p24-tab-active').removeClass('p24-tab-active trendactive4');
    $(this).addClass('p24-tab-active trendactive4');
    var id = $(this).attr('id');
    $('.p24-slider-cat').hide();
    $('#p24-slider-cat-trend5-'+id).show();
...
function callslider_script(id)
{
  ...
  (function (jQuery) {
  var tabCarousel = jQuery('#p24-slider-cat-trend5-'+id);
